        ...
        foreach ($minecrafts as $minecraft) {
            try {
                ...
                foreach($server->getWorlds() as $world) {
                    $selectOptions[$minecraft->getObjectID() . ":" . $world['name']] = '&nbsp;&nbsp;&nbsp;&nbsp;' . StringUtil::encodeHTML($world['title'] ?? $world['name']);
                    foreach ($world['maps'] as $map) {
                        $selectOptions[$minecraft->getObjectID() . ":" . $world['name'] . ":" . $map['name']] =  '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' . StringUtil::encodeHTML($map['title'] ?? $map['name']);
                    }
                }
            } catch (Exception $e) {
                // do nothing

	...
	initialize: function() {
			...
			$.each(world.maps, function(mapindex, map) {
				...
				map.element = $('<li/>')
					...
					.append($('<a/>')
							...
							.addClass('maptype')
							.css({ backgroundImage: 'url(' + (map.options.icon || (window.WCF_PATH + 'js/3rdParty/dynmap/images/block_' + worldName + '_' + mapName + '.png')) + ')' })
		...
		*/
		...
		$.each(configset, function(type, configlist) {
			...
			loadjs(window.WCF_PATH + 'js/3rdParty/dynmap/' + type + '.js', function() {
	...
	addPlayer: function(update) {
		...
		var menuitem = player.menuitem = $('<li/>')
			...
			.append(playerIconContainer = $('<span/>')
				.addClass('playerIcon')
				.append($('<img/>').attr({ src: `${window.WCF_PATH}js/3rdParty/dynmap/images/player_face.png` })) // TODO
